...
if (isset($_POST['search'])) {
  ...
  if(empty($_POST['search-box'])) {
    ...
    $sql .= "WHERE Quotation_ID = '{$result}'";
    $sql .= " OR Client_ID = '{$result}'";
    $sql .= " OR Product_ID = '{$result}'";
    $sql .= " OR CO1_ID = '{$result}'";
    $sql .= " OR CO2_ID = '{$result}'";
    $sql .= " OR CO3_ID = '{$result}'";
    $sql .= " OR Lead_Time = '{$result}'";
    $sql .= " OR Tolerance_Of_Quantity = '{$result}'";
    $sql .= " OR Terms_Of_Payment = '{$result}'";
    $sql .= " OR Quotation_Validity_Period = '{$result}'";
  }
}
...
if (isset($_POST['search-date'])) {
  ...
  if (empty($_POST['search-date-box'])) {
      ...
      if(!preg_match('^\d{4}\-(0[1-9]|1[012])\-(0[1-9]|[12][0-9]|3[01])$^', $result)) {
          ...
          $sql .= "WHERE Quotation_Creation_Date = '{$result}'";
      }
  }
}
...
<html lang="en">
  ...
  <body>
    ...
    <form name="search-form" method="POST" action="quotation.php">
    ...
    <form name="search-date-form" method="POST" action="quotation.php">
    ...
    <table width="80%" cellpadding="8" cellspace="7">
        <tr class="field-names">
            ...
            <td>CO3_ID</td>
            <td>Lead_Time</td>
            <td>Tolerance_Of_Quantity</td>
            <td>Terms_Of_Payment</td>
            <td>Quotation_Validity_Period</td>
        </tr>
    
        <?php while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) { ?>
        <tr>
            <td><?php echo $row['Quotation_ID']; ?></td>
            <td><?php echo $row['Quotation_Creation_Date']; ?></td>
            <td><?php echo $row['Client_ID']; ?></td>
            <td><?php echo $row['Product_ID']; ?></td>
            <td><?php echo $row['CO1_ID']; ?></td>
            <td><?php echo $row['CO2_ID']; ?></td>
            <td><?php echo $row['CO3_ID']; ?></td>
            <td><?php echo $row['Lead_Time']; ?></td>
            <td><?php echo $row['Tolerance_Of_Quantity']; ?></td>
            <td><?php echo $row['Terms_Of_Payment']; ?></td>
            <td><?php echo $row['Quotation_Validity_Period']; ?></td>
        </tr>
        <?php } ?>
